...
export default async function (origin: string, destination: string, transit: boolean): Promise<number> {
    ...
    const res = await fetch(`https://maps.googleapis.com/maps/api/directions/json?destination=place_id:${destination}&origin=place_id:${origin}${transitString}&key=${keys.google}`)
